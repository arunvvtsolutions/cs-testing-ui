...
export const generateMetadata = async (): Promise<Metadata> => {
  ...
  const { topColleges } = await getCatgryTopCollegesList(Category.IIT, stream, SubStream.ARCHITECTURE);
  return {
...
const getData = async (): Promise<ITopcolleges> => {
  const stream = Stream.ENGINEERING;
  const data = await getCatgryTopCollegesList(Category.IIT, stream, SubStream.ARCHITECTURE);
  return data;
};
