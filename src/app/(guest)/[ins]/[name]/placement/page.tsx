...
import PlacementComponent from 'ui-component/college-placements-page';
import { getInnerPageData } from 'utils/api/common';
import { getPlacementPageData } from 'utils/api/placement';
...
const PlacementPage = async ({ params }: { params: IPageParamsProps }) => {
  const headersList = headers();
  const domain = headersList.get('x-forwarded-host') || '';
  const data = await getPlacementPageData({ params, domain });
