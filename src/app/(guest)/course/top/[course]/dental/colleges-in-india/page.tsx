...
export const generateMetadata = async ({ params }: { params: IParamsProps }): Promise<Metadata> => {
  ...
  const { topColleges } = await getCourseTopCollegesList(params.course, stream, SubStream.DENTAL, 'india');
  return {
...
const getData = async ({ params }: { params: IParamsProps }): Promise<ITopcolleges> => {
  ...
  const data = await getCourseTopCollegesList(params.course, stream, SubStream.DENTAL, 'india');
  return data;
};
