...
const TopPharmacyColleges = ({ topColleges }: { topColleges: ICollegeData }) => {
  return (
    <Page
...
TopPharmacyColleges.getLayout = function getLayout(page: ReactElement) {
  return <Layout>{page}</Layout>;
};

export const getStaticPaths: GetStaticPaths = async () => {
  // Replace this with the actual categories you want to support
  const categories = ['government', 'private', 'deemed'];
  const topList = ['10'];
...
export const getStaticProps: GetStaticProps = async ({ params }: any) => {
  const data = await getTopCollegesData(5, params.category, params.top, Stream.MEDICAL);
...
export default TopPharmacyColleges;
