...
// ==============================|| TOP ARCHITECTURAL COLLEGES PAGE ||============================== //

const TopArchitectureColleges = ({ topColleges }: { topColleges: ICollegeData }) => {
...
TopArchitectureColleges.getLayout = function getLayout(page: ReactElement) {
  return <Layout>{page}</Layout>;
};
// Implement the getStaticPaths function
export const getStaticPaths: GetStaticPaths = async () => {
  // Replace this with the actual categories you want to support
  const categories = ['iit', 'nit', 'government', 'private'];
  const topList = ['10'];
...
export const getStaticProps: GetStaticProps = async ({ params, req }: any) => {
  const stream = Stream.ENGINEERING;
  const data = await getTopCollegesData(4, params.category, params.top, stream);
...
export default TopArchitectureColleges;
