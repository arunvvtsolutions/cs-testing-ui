...
const NavCollapse = ({ menu, level, parentId }: NavCollapseProps) => {
  ...
  const menus = menu.children?.map((item) => {
    switch (item.type) {
      ...
      case 'item':
        return <NavItem isParents key={item.id} item={item} level={level + 1} parentId={parentId} menu={menu} />;
      default:
        return (
  ...
  const isSelected =
    selected === menu.id ||
    (menu.id === 'predictor' &&
      (selectedItem.includes(PREDICTOR_RESULT.AIQ_PATH) || selectedItem.includes(PREDICTOR_RESULT.STATE_PATH)));
  ...
  const menuIcon = menu.icon ? (
    <Icon
      ...
      style={{
        color: isSelected ? 'rgb(4 156 120)' : theme.palette.text.primary
      }}
    />
  ) : (
            ...
            {menuIcon && (
              <ListItemIcon
                sx={{
                  ...
                  '&:hover': {
                    bgcolor: theme.palette.mode === 'dark' ? '#049c781a' : '#049c781a'
                  }
                }}
              >
                {menuIcon}
