...
// project imports
import { IconChevronDown, IconChevronRight, IconMinusVertical, IconLineDotted } from '@tabler/icons-react';
...
const NavGroup = ({ item, lastItem, remItems, lastItemId }: NavGroupProps) => {
  ...
  const itemIcon = currentItem?.icon ? <IconLineDotted stroke={1.5} size="20px" /> : null;
    ...
    <>
      {layout === LAYOUT_CONST.VERTICAL_LAYOUT || (layout === LAYOUT_CONST.HORIZONTAL_LAYOUT && matchDownMd) ? (
        <>
          ...
          {/* group divider */}
          {drawerOpen && item.type !== 'end' && <Divider sx={{ mt: 0.25, mb: 1.25 }} />}
        </>
      ) : (
        <List>
          <ListItemButton
